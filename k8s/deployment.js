...
apply: function(req,res){
	...
	.then(name =>{
		...
		return new Promise((resolv,reject) => {
			/* Generamos el fibercorpID si es un alta. Hay pocas probabilidades de
 			 * que se repita. Pero habria que mejorarlo. */
			if(alta)
				diccionario.push({'regex':'_fibercorpID_','value':helper.makeid(32)})
			else
				diccionario.push({'regex':'_fibercorpID_','value':req.body.fibercorpID})
...
show: function(req,res){
	...
	.then(ok => {
		...
		console.log("fibercorpID=" +  deployment.metadata.labels.fibercorpDeploy)
